...
namespace ispd::global_metrics {
    ...
    /// \brief Purge the old node-level aggreated report files.
    ///
    /// This method will remove all the necessary previously generated old node-level
    /// aggregated report files. This is done to prevent the global-level aggretation
    /// to use old metrics generated by previous simulations.
    ///
    /// It must be noted that, although it there can be 64 node-level aggregated report
    /// files, if the current simulation is using only 16 nodes, then only the 16 node-level
    /// aggreated report files will be removed.
    ///
    /// On the other hand, if a previous simulation only used 24 nodes, and the current one
    /// is using 64 nodes, then all the previously node-level aggreated report files will
    /// be removed.
