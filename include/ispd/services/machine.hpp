...
struct machine_prices {
  double cpu_individual_cost;     ///< Cost for individual cpu.
  double memory_individual_cost;  ///< Cost by GB.
  double storage_individual_cost; ///< Cost by GB.
};
struct machine_state {
  ...
  machine_prices prices;               ///< Machine's rental price.
...
struct machine {
  ...
  static void forward(machine_state *s, tw_bf *bf, ispd_message *msg,
    ...
    /// Checks if the the task is carrying a virtual machine. If so, it is
    /// forwarded to the function responsible for dealing with allocation.
    if (msg->is_vm) {
    ...
    if (msg->task.m_Dest == lp->gid || msg->application.m_Dest == lp->gid) {
      ...
      const double proc_time = s->conf.timeToProcess(
          proc_size, msg->task.m_CommSize, msg->task.m_Offload);
    ...
    /// destination.
      ...
      tw_event *const e =
          tw_event_new(route->get(msg->route_offset), g_tw_lookahead, lp);
      ispd_message *const m = static_cast<ispd_message *>(tw_event_data(e));
  ...
  static void reverse(machine_state *s, tw_bf *bf, ispd_message *msg,
    ...
    const auto start = std::chrono::high_resolution_clock::now();
#endif // DEBUG_ON
       /// Check if it's a virtual machine task.
    if (msg->is_vm) {
  ...
  static void finish(machine_state *s, tw_lp *lp) {
    ...
    ispd::node_metrics::notifyMetric(
        ...
        s->conf.getPower() + s->conf.getGpuPower());
    ispd::node_metrics::notifyMetric(
        ispd::metrics::NodeMetricsFlag::NODE_TOTAL_CPU_CORES,
        s->conf.getCoreCount());
    ispd::node_metrics::notifyMetric(
        ispd::metrics::NodeMetricsFlag::NODE_TOTAL_GPU_CORES,
        s->conf.getGpuCoreCount());
    ispd::node_metrics::notifyMetric(
    ...
    ispd::node_metrics::notifyMetric(
        ispd::metrics::NodeMetricsFlag::NODE_TOTAL_CPU_COST,
        s->m_Metrics.m_TotalCpuCost);
    ispd::node_metrics::notifyMetric(
        ispd::metrics::NodeMetricsFlag::NODE_TOTAL_MEMORY_COST,
        s->m_Metrics.m_TotalMemoryCost);
    ispd::node_metrics::notifyMetric(
        ispd::metrics::NodeMetricsFlag::NODE_TOTAL_DISK_SPACE_COST,
        s->m_Metrics.m_TotalDiskSpaceCost);
    ...
    std::printf(
        ...
        " - Total allocated vms.: %u vms (%u). \n"
        " - Total cpu cost......: %lf $ (%u). \n"
        " - Total memory cost...: %lf $ (%u). \n"
        " - Total disk space....: %lf $ (%u). \n"
        ...
        s->m_Metrics.m_AllocatedVms, lp->gid, s->m_Metrics.m_RejectedVms,
        lp->gid, s->m_Metrics.m_TotalCpuCost, lp->gid,
        s->m_Metrics.m_TotalMemoryCost, lp->gid,
        s->m_Metrics.m_TotalDiskSpaceCost, lp->gid);
  }
...
private:
  static void forward_vm(machine_state *s, tw_bf *bf, ispd_message *msg,
    ...
    if (msg->task.m_Dest == lp->gid) {
      ...
      if (vm_memory <= s->conf.getAvailableMemory() &&
          vm_storage <= s->conf.getAvailableDiskSpace() &&
          vm_cores <= s->conf.getCoreCount()) {
        fit = true;

        s->m_Metrics.m_AllocatedVms++;
        s->m_Metrics.m_TotalCpuCost += s->prices.cpu_individual_cost * vm_cores;
        s->m_Metrics.m_TotalMemoryCost +=
            s->prices.memory_individual_cost * vm_memory;
        s->m_Metrics.m_TotalDiskSpaceCost +=
            s->prices.storage_individual_cost * vm_storage;

        s->conf.setAvailableMemory(s->conf.getAvailableMemory() - vm_memory);
        s->conf.setAvailableDiskSpace(s->conf.getAvailableDiskSpace() -
                                      vm_storage);
        s->conf.setCoreCount(s->conf.getCoreCount() - vm_cores);

        s->vms.push_back(msg->vm_id);
      }
      const double proc_size = msg->task.m_ProcSize;
      const double proc_time = s->conf.timeToProcess(
          proc_size, msg->task.m_CommSize, msg->task.m_Offload);
   ...
   */
  static void sent_to_vm(machine_state *s, tw_bf *bf, ispd_message *msg,
                         tw_lp *lp) {
    auto iter = std::find(s->vms.begin(), s->vms.end(), msg->vm_id);
    if (iter != s->vms.end()) {
      ...
      *m = *msg;
      ...
      ispd_error("Virtual machine %lu not found on machine %lu", msg->vm_id,
                 lp->gid);
  }
};
