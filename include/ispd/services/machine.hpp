...
struct machine {
  ...
  /// Checks if the the task is carrying a virtual machine. If so, it is forwarded
  /// to the function responsible for dealing with allocation.
    if (msg->is_vm){
  ...
  static void finish(machine_state *s, tw_lp *lp) {
    ...
    ispd::node_metrics::notifyMetric(ispd::metrics::NodeMetricsFlag::NODE_TOTAL_CPU_COST, s->m_Metrics.m_TotalCpuCost);
    ispd::node_metrics::notifyMetric(ispd::metrics::NodeMetricsFlag::NODE_TOTAL_MEMORY_COST, s->m_Metrics.m_TotalMemoryCost);
    ispd::node_metrics::notifyMetric(ispd::metrics::NodeMetricsFlag::NODE_TOTAL_DISK_SPACE_COST, s->m_Metrics.m_TotalDiskSpaceCost);
    ...
    std::printf(
        ...
        s->m_Metrics.m_AllocatedVms, lp->gid,
        s->m_Metrics.m_RejectedVms, lp->gid,
        s->m_Metrics.m_TotalCpuCost, lp->gid,
  ...
  static void forward_vm(machine_state *s, tw_bf *bf, ispd_message *msg,
    ...
    if (msg->task.m_Dest == lp->gid) {
      ...
      if (vm_memory <= s->conf.getAvailableMemory() &&
        ...
        s->conf.setAvailableDiskSpace(s->conf.getAvailableDiskSpace() - vm_storage);
  ...
  static void sent_to_vm(machine_state *s, tw_bf *bf, ispd_message *msg, tw_lp *lp)
  {
    ...
    if (iter != s->vms.end()){
      ...
      *m = *msg;
      ...
      ispd_error("Virtual machine %lu not found on machine %lu", msg->vm_id, lp->gid);
  }
};
