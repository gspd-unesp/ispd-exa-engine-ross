...
struct route {
  ...
  inline tw_lpid get(const std::size_t index) const {
    DEBUG({
        ...
        if (index >= length) {
          std::printf("Accessing an invalid route element. (Index: %zu, Length: %u).\n",
              index, length);
          abort();
        }
    });
...
struct routing_table {
  ...
  void load(const std::string& filepath) {
    ...
    if (!file.is_open()) {
      std::cout << "Routing file " << filepath << " could not be opened." << std::endl;
      abort();
    }
...
private:
    ...
    route *parse_route_line(const std::string &routeLine,
                            tw_lpid           &src,
                            tw_lpid           &dest)
    {
